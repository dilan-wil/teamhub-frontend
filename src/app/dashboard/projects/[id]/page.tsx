...
import {
  useProject, useProjectTasks, useUpdateProject, useDeleteProject,
  useCreateTask, useUpdateTask, useDeleteTask,
} from '../hooks/use-queries';
import { useParams, Link, useLocation } from 'wouter';
import { AvatarGroup, StatusBadge, PriorityBadge } from '@/components/ui-components';
import { format } from 'date-fns';
import {
  Calendar, Users, CheckSquare, Clock, ArrowLeft, MoreHorizontal,
  Plus, Pencil, Trash2, UserPlus, Settings, UploadCloud, AlertTriangle,
...
import { CreateTaskDialog, EditTaskDialog } from '@/components/dialogs/task-dialog';
...
export default function ProjectDetail() {
  ...
  const handleAddMember = async (userId: string) => {
    ...
    await updateProject.mutateAsync({
      id: project.id,
      data: { members: [...project.members, user] },
    });
  };
  ...
  const handleRemoveMember = async (memberId: string) => {
    await updateProject.mutateAsync({
      id: project.id,
      data: { members: project.members.filter(m => m.id !== memberId) },
    });
  };
  ...
  const handleDeleteTask = async () => {
    ...
    await delpoeteTask.mutateAsync(selectedTask.id);
  ...
  return (
    <PageTransition className="space-y-6 pb-12">
      ...
      {/* Hero */}
      <div className={cn('relative rounded-3xl overflow-hidden shadow-md bg-gradient-to-br min-h-[220px] flex flex-col justify-end p-8 text-white', project.gradient)}>
        <div className="absolute inset-0 bg-black/25" />
        <div className="relative z-10 flex justify-between items-end gap-4">
          <div className="max-w-2xl">
            <StatusBadge status={project.status} className="mb-3 bg-white/20 border-white/30 text-white" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: CheckSquare, color: 'text-primary bg-primary/10', label: 'Progress', value: `${project.progress}%` },
          { icon: Users, color: 'text-emerald-600 bg-emerald-500/10', label: 'Team', value: `${project.members.length} members` },
          { icon: Clock, color: 'text-amber-600 bg-amber-500/10', label: 'Timeline', value: `${daysLeft} days left` },
          { icon: Calendar, color: 'text-purple-600 bg-purple-500/10', label: 'Due Date', value: format(new Date(project.dueDate), 'MMM d, yyyy') },
        ].map(({ icon: Icon, color, label, value }) => (
          <div key={label} className="bg-card border border-border/50 rounded-2xl p-5 flex items-center gap-4 shadow-sm">
            <div className={cn('p-3 rounded-xl', color)}><Icon className="w-5 h-5" /></div>
            <div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm">
                  ...
                  <div className="h-3 w-full bg-muted rounded-full overflow-hidden">
                    <motion.div
                      className={cn('h-full rounded-full bg-gradient-to-r', project.gradient)}
                      initial={{ width: 0 }}
                  ...
                  <div className="flex gap-4 mt-4">
                    {(['todo', 'in-progress', 'review', 'done'] as const).map(s => (
                      <div key={s} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">{taskCounts[s]}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm">
                  ...
                  {tasksLoading ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground text-sm">
                      No tasks yet.{' '}
                      <button onClick={() => setShowCreateTask(true)} className="text-primary hover:underline font-medium">
                        Create one
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {tasks.slice(0, 6).map(task => (
                        <div key={task.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/40 transition-colors">
                    ...
                    <tbody className="divide-y divide-border/50">
                      {filteredTasks.map(task => (
                        <tr key={task.id} className="hover:bg-muted/30 transition-colors group">
                          <td className="px-5 py-3.5 font-medium max-w-[200px] truncate">{task.title}</td>
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm truncate max-w-[90px]">{task.assignee.name}</span>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3.5 text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="rounded-xl w-36">
                                <DropdownMenuItem onClick={() => { setSelectedTask(task); setShowEditTask(true); }} className="rounded-lg cursor-pointer">
                                  <Pencil className="w-3.5 h-3.5 mr-2" /> Edit
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => { setSelectedTask(task); setShowDeleteTask(true); }}
          ...
          {activeTab === 'members' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">{project.members.length} member{project.members.length !== 1 ? 's' : ''} in this project</p>
                <button
              ...
              {project.members.length === 0 ? (
                <div className="bg-card border border-border/50 rounded-3xl py-16 flex flex-col items-center text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">No members in this project yet.</p>
                  <button onClick={() => setShowAddMember(true)} className="text-sm text-primary font-medium hover:underline">
          ...
          {activeTab === 'settings' && (
            <ProjectSettingsTab
              project={project}
              onSaved={() => { setSavedSettings(true); setTimeout(() => setSavedSettings(false), 2500); }}
      ...
      {showEditTask && selectedTask && (
        <EditTaskDialog task={selectedTask} open={showEditTask} onOpenChange={setShowEditTask} />
      )}
      <DeleteConfirmDialog
        title="Delete Task"
...
function ProjectSettingsTab({
  ...
  const updateProject = useUpdateProject();
  const { register, handleSubmit, control, reset, formState: { errors, isDirty } } = useForm<SettingsValues>({
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <form onSubmit={handleSubmit(onSubmit)} className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm space-y-5">
        ...
        <div>
          ...
          <Textarea {...register('description')} rows={3} className="rounded-xl resize-none" />
      ...
      <div className="bg-card border border-red-500/20 rounded-3xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Deleting this project is permanent and cannot be undone. All tasks will be removed.
        </p>
        <button
          type="button"
